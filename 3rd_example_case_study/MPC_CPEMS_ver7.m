...
DeltaT= 0.25;
eta_c= eta_c* DeltaT; %charging in efficency
eta_d= eta_d* DeltaT; %charging in efficency

Emax=0.85*50; %maximum energy in battery
Emin=0.15*50; %minimum energy in battery
Pmax_inverter=5;
...
c=c';
% c= repmat(c,1,210);
% P_solar= repmat(P_solar,1,210);
% P_solar_spv= repmat(P_solar_spv,1,210);

P_solar= table2array(fifteen_minute_data(1:size(AD_matrix,2),3))*0; %100 m^2
P_solar= P_solar';
P_solar_spv= table2array( fifteen_minute_data(1:size(AD_matrix,2),3))* 2 ; % 1m^2
P_solar_spv= P_solar_spv';
P_solar_spv= repmat(P_solar_spv, N,1);

P_solar_spv= P_solar_spv.*AD_matrix;
%SPEV portion
SPEV_Portion= 0;
EV_Portion=1- SPEV_Portion;
num_rows= round(size(P_solar_spv, 1)*EV_Portion);
remained_rows= size(P_solar_spv, 1) - num_rows;
modification_mat= [ones(num_rows, size(P_solar_spv, 2))* 0.0001; ones(remained_rows, size(P_solar_spv, 2))];
P_solar_spv= P_solar_spv.*modification_mat;
...
leng_s= sequences(car, 2)- sequences(car, 1)+1;
sty= sequences(car, 1): sequences(car, 2);
last_stp= sequences(car, 2);
E_goal= 0.8* min(Emax,  Emin+ eta_c* Pmax_inverter* leng_s);
E_goal_matrix(pile, last_stp)= E_goal;
        for t= sty(1:end-1)
            leng_s= leng_s-1;
            minE=  max(Emin, E_goal- leng_s* eta_c* Pmax_inverter);
            E_goal_matrix(pile, t)=minE;
...
disp(stp);
    ...
    % Validate sizes and check for NaN or Inf values
    % assert(all(isfinite(f)), 'Objective function contains NaN or Inf');
    % assert(all(isfinite(b)), 'Constraints vector contains NaN or Inf');
    % assert(all(isfinite(ub)), 'Bounds contain NaN or Inf');
    % assert(all(isfinite(lb)), 'Bounds contain NaN or Inf');
